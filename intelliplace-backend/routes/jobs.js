...
router.post('/', authenticateToken, authorizeCompany, upload.single('jobDescriptionFile'), async (req, res) => {
  try {
    ...
    let title, description, location, type, salary, requiredSkills, minCgpa, includeCgpaInShortlisting, allowBacklog, maxBacklog, deadline;
    ...
    if (req.file) {
      ...
      ({ title, description, location, type, salary, requiredSkills, minCgpa, includeCgpaInShortlisting, allowBacklog, maxBacklog, deadline } = req.body);
    }
...
router.post('/:jobId/close', authenticateToken, authorizeCompany, async (req, res) => {
  try {
    ...
    if (job.status === 'CLOSED') {
      return res.status(400).json({ success: false, message: 'Applications for this job are already closed' });
    }
    ...
    res.status(500).json({ 
      success: false, 
      message: 'Server error closing applications' 
    });
  }
});
